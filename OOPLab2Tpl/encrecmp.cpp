...
int main()
{
    ...
    for (i = 0; i < 8; i++)
        for (l = 0; l < 8; l++)
        {
            r = i; // 0000 0000 0000 0iii <- i
            w = l << 3;
            r |= w; // 0000 0000 00ll l000 <- l
            b = 0;
            t = 1;
            ...
            w = 1 << 6;
            if (b)
                r |= w; // 0000 0000 0pll liii <- b
            //    kccc  cccc cpll liii
            // @return 
            w = s[i][l];  //0000 0000 cccc cccc
            w <<= 7;           //0ccc cccc c000 0000
            r |= w;
            if (r & 0x4000)    //0100 0000 0000 0000
                r |= 0x8000;   //1000 0000 0000 0000 
            sh_dat[j] = r;
            j++;
            ofs << hex << r << ' ';
            cout << hex << r << endl;
